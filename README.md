...
| `scoreboard_button_dinput_index` | controller button toggle (DirectInput fallback) | a plain number, e.g. `2`. Used when your controller is NOT found via XInput — e.g. Steam Input on Windows only remaps input for the specific game it's actively hooking, not system-wide like on Linux, so a raw DirectInput pad it isn't remapping falls back to this. Not set by default |
...
3. win_controller.py - this helps us determine when the scoreboard is open. Tries XInput first (real Xbox pads, or Steam Input set to Xbox emulation); if nothing's found there, falls back to Windows' legacy DirectInput-based joystick API, which sees raw controllers directly without needing Steam Input to remap them at all (Steam Input on Windows only remaps for the specific game it's hooking, not system-wide). The scoreboard button is by default L3/LS, but its configurable in the config/first time setup.
