...
def _overlay_env(ui_scale: float | None = None) -> dict[str, str]:
    ...
    if ui_scale is not None:
        # Overlay reads $RL_UI_SCALE as a fallback below rl_ui_scale in
        # config.json (see config._load_ui_scale in gtk4_overlay.py) — so
        # this overrides config.json for this run only, without touching
        # the file itself.
