...
scale. If your desktop resolution or RL's UI scale differs, the
positions won't line up — see NOTES at the bottom for how to
recalibrate.
...
BOX_SIZE = 51
SLOT_X = 712  # horizontal position — same for both teams, all sizes
...
def get_scoreboard_slots(team_size: int) -> list:
    ...
    size (1-4), using the calibrated layout closest to it."""
    ...
    for row in range(team_size):
        slots.append(("blue", row, SLOT_X, blue_row0_y + row * ROW_HEIGHT, BOX_SIZE, BOX_SIZE))
    for row in range(team_size):
        slots.append(("orange", row, SLOT_X, orange_row0_y + row * ROW_HEIGHT, BOX_SIZE, BOX_SIZE))
    return slots

# Goal-scored nameplate avatar box.
# Reverted to your last confirmed-good manual calibration.
GOAL_NAMEPLATE_X_NUDGE = 0
GOAL_NAMEPLATE_Y_NUDGE = -38
GOAL_NAMEPLATE_SLOT = (
...
#   The SCOREBOARD_SLOTS and GOAL_NAMEPLATE_SLOT coordinates above are
#   only valid at REFERENCE_RESOLUTION + REFERENCE_UI_SCALE. Layer 5
#   will replace the hardcoded x/y with a scale_position() helper that
#   uses hyprctl's actual RL window geometry, so this file will need a
#   small refactor once that lands — for now it assumes your desktop
#   is running RL at exactly 2560x1440 with 75% UI scale, undecorated
#   borderless windowed.
#
# Click-through:
